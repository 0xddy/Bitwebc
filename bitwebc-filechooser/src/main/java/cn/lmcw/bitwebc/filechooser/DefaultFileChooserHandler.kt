...
open class DefaultFileChooserHandler(
    ...
    private suspend fun openVideoCapture(launcher: FileChooserLauncherRegistry) {
        val granted = requestPermissionSuspend(launcher, Manifest.permission.CAMERA)
        if (!granted) {
            eventReporter?.invoke(BitwebcEvent.FileChooserPermissionDenied("CAMERA 权限被拒绝"))
            dispatchResult(null)
    ...
    private suspend fun openAudioCapture(launcher: FileChooserLauncherRegistry) {
        val granted = requestPermissionSuspend(launcher, Manifest.permission.RECORD_AUDIO)
        if (!granted) {
            eventReporter?.invoke(BitwebcEvent.FileChooserPermissionDenied("RECORD_AUDIO 权限被拒绝"))
            dispatchResult(null)
            return
        }
